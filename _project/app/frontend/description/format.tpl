...
<ul>
    ...
    <li>Если Вы хотите изменить принципы определения view в целом для всего проекта, - Вы можете создать свой класс \fan\project\view\definer, унаследовав его от \fan\core\view\definer и переопределить в нем соответствующие методы.</li>
</ul>
...
<ul>
    ...
    <li><b>\core\view\router</b> - определяет какой тип view использовать для показа результатов данного запроса (<i>класс может быть переопределен для некоторых <span class="keyword">форматов view</span></i>). По умолчанию, для всех видов данных, для которых не указан специальный router - используется \fan\core\view\router\simple;</li>
...
<p>
    В рамках данного теста создано два дополнительных нестандартных парсера: \fan\project\view\parser\custom1 и \fan\project\view\parser\custom2. Как написано выше, для использования этих парсеров у основного блока задан метод "getViewParserName". Оба этих класса должны иметь методы: <b>статический getType</b>, возвращающий название данного парсера и <b>getFinalContent</b>, возвращающий конечный текст на основе данных из view-keeper данного блока. Рассмотрите исходный код названых классов, для анализа принципов их работы.
</p>
{/if}

<article>
    <h1 class="header">PHP-FAN.5: Test of embeded block and view format <span>/ PHP-FAN.5: Проверка вложеных блоков и формата view</span></h1>

    <div class="result">
        <dl>
            <dt>User_Agent: </dt> <dd>{$sUserAgent}</dd>
        </dl>
        {$extra}
    </div>
    ...
    <p>
        <hr />
        Если Вы только осваиваете PHP-FAN, Вам лучше вернуться к этому тесту позже. Но если Вы хотите прямо сейчас освоить более сложные операции с view - кликните <a href="?advanced=1">здесь</a>.
    </p>
    {else}
    ...
    <div class="add_tasks">
        ...
        <ul>
            <li>Изучите метод "getViewParserName" созданный в этом блоке. Посмотрите как здесь определяется custom-views, для которых нет правил в config-файле. Рассмотрите как определяется формат view в этом методе, если условия, указанные для custom, не выполняются.</li>
            <li>Попробуйте указать различные значения по-умолчанию для формата view в мета-файле текущего блока (раскомментируйте значение default_view_format).</li>
            <li>Попробуйте изменять правила определения формата view в конфигурационном файле и произвести обращение .</li>
            <li>Попробуйте создать свой custom-view или попросту именить те custom-view что даны здесь в качестве примера.</li>
        </ul>
    </div>
    {/if}
</article>
